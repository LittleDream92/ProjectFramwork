...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(i*kScreenWidth/3, 40, (kScreenWidth-3)/3, 80)];
            backView.tag = 100+i;
            [backView addGestureRecognizer:tap];
            [self addSubview:backView];
            //
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, (kScreenWidth-3)/3, 50)];
            imageView.contentMode = UIViewContentModeScaleAspectFit;
            imageView.tag = i+20;
            [backView addSubview:imageView];
            //
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(i*kScreenWidth/3-1, 45, 0.5, 65)];
